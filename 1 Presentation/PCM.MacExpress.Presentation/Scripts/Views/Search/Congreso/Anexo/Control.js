...
/// </remarks>
ns('PCM.Reclamo.Presentacion.Search.Congreso.Anexo');
PCM.Reclamo.Presentacion.Search.Congreso.Anexo.Controller = function () {
    var base = this;
    base.Ini = function () {
        'use strict'
    ...
    base.Event = {
        ...
        BtnDescagarDocumentoAnexoClick: function (thiss) {
            ...
            if (filtro.NroDocumento != null) {
                Gob.Pe.UI.Web.Components.Util.RedirectPost(PCM.Reclamo.Presentacion.Search.Congreso.Actions.DescargarDocumentoAnexo, filtro);
            }
            else {
                ...
                if (datas.Result[2].IsSuccess) {
                    ...
                    if (datas.Result[2].Result  != null) { 
                        $.each(datas.Result[2].Result , function (index, value) {
                            ...
                            tr.append("<td style='width:40px;text-align: center;'  onclick='PCM.Reclamo.Presentacion.Search.Congreso.Anexo.Vista.Event.BtnDescagarDocumentoAnexoClick(this);' anexo='" + value.NroDocumento + "' ><span class='glyphicon glyphicon-folder-open' title='" + PCM.Reclamo.Presentacion.Base.GenericoResource.EtiquetaVerDocumento + "'></span></td>");
        ...
        AjaxAnexoDetalleSuccess: function (datas) {
            ...
            if (datas.IsSuccess) {
                ...
                if (datas.Result[1].IsSuccess) {
                    base.Control.TableAnexosBody().empty();
                    if (datas.Result[1].Result[0] != null && datas.Result[1].Result[0].Length > 0) {
                        $.each(datas.Result[1].Result[0], function (index, value) {
                            var tr = $('<tr/>');
                            tr.append("<td>" + value.Descripcion + "</td>");
                            tr.append("<td>" + value.NombreAnexo + "</td>");
                            tr.append("<td style='width:40px;'><span class='glyphicon glyphicon-folder-open' title='" + PCM.Reclamo.Presentacion.Base.GenericoResource.EtiquetaVerDocumento + "'></span></td>");
                            base.Control.TableAnexosBody().append(tr);
                        });
                    }
                    else {
    ...
    base.Ajax = {
        ...
        AjaxCargarAnexo: new Gob.Pcm.UI.Web.Components.Ajax({
            action: PCM.Reclamo.Presentacion.Search.Congreso.Actions.AnexoFlujo,
            autoSubmit: false,
            onSuccess: base.Event.AjaxCargarAnexoSuccess
        }),
        AjaxAnexoDetalle: new Gob.Pcm.UI.Web.Components.Ajax({
            action: PCM.Reclamo.Presentacion.Search.Congreso.Actions.AnexoDetalle,
            autoSubmit: false,
...
try {
    $(document).ready(function () {
        'use strict';
        PCM.Reclamo.Presentacion.Search.Congreso.Anexo.Vista = new PCM.Reclamo.Presentacion.Search.Congreso.Anexo.Controller();
        PCM.Reclamo.Presentacion.Search.Congreso.Anexo.Vista.Ini();
    });
} catch (ex) {
}